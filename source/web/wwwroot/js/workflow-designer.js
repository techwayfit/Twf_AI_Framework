...
let selectedNodes = new Set(); // Multi-selection support
let selectedVariable = null; // Currently selected variable in sidebar
...
let activeAutocomplete = null; // Currently active autocomplete dropdown
let activeSidebarTab = 'nodes'; // Current sidebar tab
...
// Render parameter field with validation and autocomplete
...
// Update node parameter functions
...
function selectNodesInRect(rectLeft, rectTop, rectWidth, rectHeight) {
  const rectRight = rectLeft + rectWidth;
    ...
    workflow.nodes.forEach(node => {
        ...
        const nodeRight = nodeLeft + 180; // Approximate node width
        const nodeBottom = nodeTop + 80; // Approximate node height
...
function onCanvasDrop(event) {
  ...
  // Calculate position relative to canvas scroll - center the node at drop point
    const x = (event.clientX - rect.left + canvas.scrollLeft) / zoomLevel - 80; // 80 is approx half node width
    const y = (event.clientY - rect.top + canvas.scrollTop) / zoomLevel - 40; // 40 is approx half node height
 ...
 if (e.ctrlKey) {
   ...
   isDraggingNode = true;
     ...
     const canvasRect = canvas.getBoundingClientRect();
        ...
        // Calculate offset from mouse to THIS node's position (not the primary selected node)
