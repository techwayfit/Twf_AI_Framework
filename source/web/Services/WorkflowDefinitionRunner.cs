...
public sealed class WorkflowDefinitionRunner
{
    ...
    /// </summary>
    public async Task<WorkflowRunResult> RunWithCallbackAsync(WorkflowDefinition definition, WorkflowData? initialData, Func<NodeStepEvent, Task> onStep)
    {
        // Validate workflow has a Start node
        var startNode = definition.Nodes.FirstOrDefault(n => n.Type == "StartNode") ?? throw new InvalidOperationException(
                        $"Workflow '{definition.Name}' has no Start node. Drag a Start node onto the canvas and save before running.");
        ...
        _logger.LogInformation(
               "▶ Running workflow '{WorkflowName}' from JSON definition. Nodes: {NodeCount}",
               definition.Name,
        ...
        {
            _logger.LogInformation("✅ Workflow '{WorkflowName}' completed successfully", definition.Name);
            return WorkflowRunResult.Success(definition.Name, walkResult.Data);
        }
        else
        {
            _logger.LogError("❌ Workflow '{WorkflowName}' failed at '{FailedNode}': {Error}", definition.Name,
              walkResult.FailedNodeName ?? "unknown", walkResult.ErrorMessage);
    ...
    public Task<WorkflowRunResult> RunAsync(WorkflowDefinition definition, WorkflowData? initialData = null)
    ...
    /// </summary>
    private static Dictionary<(Guid nodeId, string port), Guid> BuildRouting(List<ConnectionDefinition> connections)
    {
